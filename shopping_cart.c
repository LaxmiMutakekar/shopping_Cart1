...
void updatebill()//function for updating the bill after each perchase or delete
{
    int i;
    total=0;
    ...
    for(i=0;i<inum;i++)
        total=total+c[i].cost;//calculates the cart total
    return;
}
...
void displaycart()//function for displaying the cart items
{
    int i;
    ...
    for(i=0;i<inum;i++)
        printf("\n%d\t\t%d\t\t%5.2f",c[i].code,c[i].quantity,c[i].cost);//prints all cart //elements
  printf("\n\n\n\t\t\tGrand total is:%5.2f\n",total);
    return;
}
